...
#     Everything below runs ONLY as a script. Previously it executed at import
#     time, so `import pull_daily` (e.g. to reuse a helper, or a stray import
#     in a review/analysis script) rewrote output/daily_README.md and the
#     integrity report as a side effect -- the same defect that was fixed in
#     pull_returns.py.
# ---------------------------------------------------------------------------
def main() -> None:
    prices = _load_cache()
